...
public class MandelbrotGenerator {
    public Canvas create(int iterations, int zoom, int YMov, int XMov, String gradient) {
        Canvas canvas = new Canvas(450, 400);
        PixelWriter pw = canvas.getGraphicsContext2D().getPixelWriter();
        drawSet(pw, zoom, XMov, YMov, iterations, gradient);

        return canvas;
    }

    public void drawSet(PixelWriter pw, double zoom, int XOff, int YOff, int iterations, String gradient) {
        double cx, cy;
